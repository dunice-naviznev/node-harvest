...
TaskAssignment.prototype.createAndAssign = function (options, cb) {
    if (_isUndefined(options, 'project_id')) {
        return cb(new Error('creating and assigning a task assignment requires an id'));
    }

    var url = '/projects/' + options.project_id + '/task_assignments/add_with_create_new_task';
